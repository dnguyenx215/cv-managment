...
namespace CVManagementAPI.Services
{
    public class AccountService
    {
        ...
        public async Task<PageResult<AccountDTO>> GetAllPagination(Pagination? pagination)
        {
            var entities = GetAllAccount();
            var result = PageResult<AccountDTO>.ToPageResult(pagination, entities);
            pagination.TotalCount = entities.Count();
            return new PageResult<AccountDTO>(pagination, result);
        }
    }
}