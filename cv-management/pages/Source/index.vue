<template>
    <div class="row">
        <div class="p-4">
            <div class="search">
                <h4 class="search-text h5">Nguồn CV tuyển dụng</h4>
            ...
            <div class="data">
                <div class="btn-handle d-flex justify-content-end mb-3">
                    <div class="handle-create">
                        <TheFormCreateUpdateSource
                            :isEditMode="isEditMode"
                            :editSource="editSourceData"
                            @source-saved="getSources"
                        />
                        <a
                            @click="onCreateMode"
                            href="#"
                            ...
                            data-bs-target="#create-update-source-modal"
                        >
                            <svg
                                xmlns="http://www.w3.org/2000/svg"
                                width="2em"
                    ...
                    <div class="reload">
                        <a class="text-info" href="#" @click="getSources()">
                            <svg
                                xmlns="http://www.w3.org/2000/svg"
                                width="2em"
                                height="2em"
                ...
                <table class="table text-center table-hover">
                    <thead class="table-active table-primary">
                        <tr>
                            <th scope="col">No.</th>
                            <th scope="col">Tên nguồn CV tuyển dụng</th>
                    ...
                    <tbody class="table-success">
                        <tr v-for="(source, index) in sources" :key="index">
                            ...
                            <td>
                                <div v-if="source.nameSource.toLowerCase() !== 'unknown'"
                ...
                <DeleteSourceModal
                    ...
                    @source-deleted="getSources"
                    @hide-modal="getSources()"
                />
            </div>
        </div>
    </div>
...
<script setup>
import { ref } from 'vue';
import DeleteSourceModal from '~/components/form/DeleteSourceModal.vue';
...
const getSources = async () => {
    try {
        const res = await api.get(`/Source`, null);
        sources.value = res.data.responseData;
    } catch (err) {
        console.log(err);
    }
};
...
const search = async () => {
    try {
        if (!searchString.value) {
            ...
            const res = await api.get(`/Source/SearchSource/${searchString.value}`, null);
            sources.value = res.data.responseData;
        }
    } catch (err) {
        console.log(err);
